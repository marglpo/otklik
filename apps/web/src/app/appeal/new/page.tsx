      ...
      <PublicShell>
        <section className="mx-auto max-w-2xl space-y-6 py-8 sm:py-12">
          <div className="surface-card p-6 sm:p-9">
            <p className="font-medium text-indigo-700">Обращение принято</p>
          ...
          <Link
            href="/appeal/current"
            className="block rounded-xl bg-indigo-700 px-5 py-3 text-center font-medium text-white"
          >
            Посмотреть статус
          </Link>
        </section>
    ...
    <PublicShell>
      <form onSubmit={submit} className="mx-auto max-w-3xl space-y-7 py-8 sm:py-12">
        <div className="space-y-3">
          <div className="flex items-center gap-3"><span className="rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-800">Шаг 1 из 1</span><span className="text-xs text-slate-500">Обычно 3–5 минут</span></div>
          <p className="text-sm font-semibold text-indigo-700">Новое обращение</p>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {reference?.applicant_types.map((item) => (
              <button
                ...
                className={`flex min-h-16 flex-col items-center justify-center rounded-xl border px-3 py-3 text-sm font-semibold ${
              ...
              >
                {item.label}
                {applicantType === item.code ? <span className="mt-1 text-xs font-medium text-indigo-100">✓ Выбрано</span> : null}
              </button>
            ))}
          </div>
        </fieldset>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {reference?.categories.map((category) => (
                <button
                  ...
                  className={`min-h-14 rounded-xl border p-3 text-left text-sm font-medium ${
