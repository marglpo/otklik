...
export default function ExpertWorkspacePage() {
  ...
  return (
    <main className="staff-workspace">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 p-4 sm:p-6 lg:p-8 xl:grid-cols-[320px_1fr]">
        <aside className="space-y-3">
          ...
          {items.map((item) => <button key={item.id} type="button" onClick={() => { void releaseComposer(); void loadDetail(item.id) }} className={`w-full rounded-2xl border p-4 text-left shadow-sm transition hover:border-indigo-300 hover:shadow-md ${detail?.id === item.id ? "border-indigo-400 bg-indigo-50 ring-2 ring-indigo-100" : item.crisis_flag || item.priority === "urgent" ? "border-red-200 bg-red-50/60" : "border-slate-200 bg-white"}`}>
            <p className="break-safe font-medium">{item.category?.name ?? "Без категории"}</p><div className="mt-3 flex flex-wrap gap-1.5"><StatusBadge value={item.status} label={statusLabels[item.status]} /><span className="inline-flex min-h-6 items-center rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600">{waiting(item.waiting_seconds)}</span>{item.priority === "urgent" ? <StatusBadge value="urgent" /> : null}{item.crisis_flag ? <StatusBadge value="crisis" label="Требует внимания" /> : null}</div>
          </button>)}
        </aside>
        <section className="min-w-0 space-y-4">
          ...
          {!detail ? <div className="staff-card p-10 text-center text-slate-500">Выберите назначенное вам обращение.</div> : <>
            ...
            <div className="grid gap-4 min-[1400px]:grid-cols-2">
              <section className="staff-card p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Диалог с заявителем</h2><span className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs text-indigo-700">Виден заявителю</span></div><div className="soft-scrollbar my-4 max-h-80 space-y-3 overflow-y-auto">{detail.messages.map((item) => <div key={item.id} className={`rounded-xl p-3 text-sm ${item.author_type === "specialist" ? "ml-8 bg-indigo-50" : "mr-8 bg-slate-100"}`}><p className="mb-1 text-xs font-medium text-slate-500">{item.author_label}</p><p className="whitespace-pre-wrap">{item.body}</p></div>)}</div><Textarea value={message} onFocus={() => void acquireComposer()} onChange={(event) => setMessage(event.target.value)} placeholder="Сообщение заявителю" maxLength={5000}/><div className="mt-3 flex flex-wrap gap-2"><Button disabled={busy || !lockHeld || !message.trim()} onClick={() => void mutate(() => expertApi.message(request, detail.id, message), () => { setMessage(""); void releaseComposer() })}>Отправить</Button><Button variant="outline" disabled={busy || !lockHeld} onClick={() => void mutate(() => expertApi.clarify(request, detail.id, message.trim() || null), () => { setMessage(""); void releaseComposer() })}>Запросить уточнение</Button></div>{!lockHeld ? <p className="mt-2 text-xs text-slate-500">Нажмите в поле, чтобы занять редактор ответа.</p> : <p className="mt-2 text-xs font-medium text-indigo-700">Редактор закреплён за вами.</p>}</section>
              <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-amber-950">Внутренние заметки</h2><span className="rounded-full border border-amber-200 bg-white px-2.5 py-1 text-xs font-semibold text-amber-900">Только для команды</span></div><p className="mt-1 text-xs text-amber-800">Не видны заявителю и не входят в диалог.</p><div className="soft-scrollbar my-4 max-h-64 space-y-2 overflow-y-auto">{detail.internal_notes.map((item) => <div key={item.id} className="rounded-xl border border-amber-100 bg-white p-3 text-sm"><p className="text-xs font-medium text-amber-800">Внутренняя заметка · {item.author_label}</p><p className="break-safe mt-1 whitespace-pre-wrap">{item.body}</p></div>)}</div><Textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Внутренняя заметка" maxLength={5000}/><Button className="mt-3" variant="outline" disabled={busy || !note.trim()} onClick={() => void mutate(() => expertApi.note(request, detail.id, note), () => setNote(""))}>Добавить заметку</Button></section>
