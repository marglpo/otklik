...
function Panel({ title, note, children }: { title: string; note?: string; children: ReactNode }) {
  return <section className="staff-card space-y-4 p-5 sm:p-6">
    <div><h2 className="text-lg font-semibold tracking-tight">{title}</h2>{note ? <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">{note}</p> : null}</div>{children}
  </section>
}
...
function CreatePanel({
  ...
  return <section className="staff-card p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold tracking-tight">{title}</h2>{note ? <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">{note}</p> : null}</div>{!open ? <Button type="button" onClick={() => setOpen(true)}>{action}</Button> : null}</div>{open ? <div className="mt-5 border-t border-slate-100 pt-5">{children(() => setOpen(false))}</div> : null}</section>
}
...
function ToggleList<T extends { id: string; is_active: boolean }>({ items, label, toggle, edit, busy }: { items: T[]; label: (value: T) => string; toggle: (value: T) => Promise<unknown>; edit?: (value: T) => void; busy: boolean }) {
  return <Panel title="Текущая конфигурация">{items.length ? <div className="divide-y">{items.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 py-3"><span>{label(item)}</span><div className="flex gap-2">{edit ? <Button size="sm" variant="outline" disabled={busy} onClick={() => edit(item)}>Изменить</Button> : null}<Button size="sm" variant="outline" disabled={busy} onClick={() => { if (window.confirm("Изменить активность записи?")) void toggle(item) }}>{item.is_active ? "Деактивировать" : "Активировать"}</Button></div></div>)}</div> : <p className="text-sm text-slate-500">Записей пока нет.</p>}</Panel>
}
...
export default function AdminWorkspacePage() {
  ...
  return <main className="staff-workspace"><StaffHeader title="Управление продуктом" role="Администратор" name={actor.display_name} onLogout={() => { void logout().finally(() => router.replace("/staff/login")) }} navigation={sections.map(([id, label]) => ({ id, label, active: section === id, onSelect: () => setSection(id) }))} />
    <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8"><div className="min-w-0 space-y-5">
...
function Overview({ analytics, rangeDays, setRangeDays, request }: { analytics?: Analytics; rangeDays: number; setRangeDays: (value: number) => void; request: StaffRequest }) {
  ...
  return <><Panel title="Аналитика" note="Только операционные метаданные и временные метрики — без расшифровки содержимого."><div className="flex flex-wrap gap-2"><Button variant={rangeDays === 7 ? "default" : "outline"} onClick={() => setRangeDays(7)}>7 дней</Button><Button variant={rangeDays === 30 ? "default" : "outline"} onClick={() => setRangeDays(30)}>30 дней</Button><Button variant="outline" disabled={!analytics} onClick={() => void download()}>Скачать CSV</Button></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{metrics.map(([name, value], index) => <div key={String(name)} className={`rounded-2xl border p-4 ${index === 0 ? "border-indigo-200 bg-indigo-50" : "border-slate-200 bg-slate-50"}`}><p className="text-sm text-slate-500">{name}</p><p className="mt-2 text-2xl font-semibold tracking-tight">{value ?? "—"}</p></div>)}</div></Panel>{analytics ? <><ChartPanel title="Обращения по дням" data={analytics.daily} kind="line" /><div className="grid gap-5 xl:grid-cols-3"><ChartPanel title="Категории" data={analytics.by_category} /><ChartPanel title="Статусы" data={analytics.by_status} /><ChartPanel title="Типы заявителей" data={analytics.by_applicant_type} /></div><Panel title="Нагрузка">{analytics.workloads.length ? analytics.workloads.map((item) => <div key={item.staff_user_id} className="flex justify-between gap-4 border-b border-slate-100 py-3 text-sm"><span>{item.display_name} · {item.role}</span><strong className="shrink-0 rounded-full bg-slate-100 px-2.5 py-1">{item.active_appeals}{item.capacity == null ? " действий" : ` / ${item.capacity}`}</strong></div>) : <p className="text-sm text-slate-500">Нет данных.</p>}</Panel></> : <Panel title="Аналитика"><p className="text-sm text-slate-500">Загрузка…</p></Panel>}</>
}
...
function ChartPanel({ title, data, kind = "bar" }: { title: string; data: Array<{ key?: string; date?: string; count: number }>; kind?: "bar" | "line" }) {
  return <Panel title={title}>{data.length ? <div className="h-64 min-w-0"><ResponsiveContainer width="100%" height="100%">{kind === "line" ? <LineChart data={data}><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tick={{ fill: "#64748b", fontSize: 11 }} /><YAxis allowDecimals={false} tick={{ fill: "#64748b", fontSize: 11 }} /><Tooltip /><Line dataKey="count" stroke="#4f46e5" strokeWidth={2} dot={false} /></LineChart> : <BarChart data={data}><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="key" hide /><YAxis allowDecimals={false} tick={{ fill: "#64748b", fontSize: 11 }} /><Tooltip /><Bar dataKey="count" fill="#6366f1" radius={[6, 6, 0, 0]} /></BarChart>}</ResponsiveContainer></div> : <p className="text-sm text-slate-500">Нет данных за период.</p>}</Panel>
}
...
function StaffView({ items, request, run, busy }: { items: StaffItem[]; request: StaffRequest; run: Run; busy: boolean }) {
  ...
  return <><CreatePanel action="Добавить сотрудника" title="Сотрудники" note="После создания временный пароль будет показан один раз.">{(close) => <form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const f = new FormData(form); const expert = newRole === "expert"; let created: StaffCreateResult | null = null; void run(async () => { created = await adminApi.createStaff(request, { login: f.get("login"), email: f.get("email"), display_name: f.get("name"), role: newRole, ...(expert ? { public_specialist_label: f.get("label"), max_active_appeals: Number(f.get("capacity") || 10) } : {}) }); return created }, "Сотрудник создан.").then((saved) => { if (saved && created) { setCredentials(created); form.reset(); setNewRole("operator"); close() } }) }}><Field label="Логин" name="login" required /><Field label="Email" name="email" type="email" required /><Field label="Отображаемое имя" name="name" required /><div><Label htmlFor="role">Роль</Label><select id="role" name="role" value={newRole} onChange={(event) => setNewRole(event.target.value as typeof newRole)} className="h-10 w-full rounded-md border px-3"><option value="operator">Оператор</option><option value="expert">Эксперт</option><option value="admin">Администратор</option></select></div>{newRole === "expert" ? <><Field label="Специализация эксперта" name="label" required /><Field label="Вместимость" name="capacity" type="number" required /></> : null}<CreateActions busy={busy} close={() => { setNewRole("operator"); close() }} /></form>}</CreatePanel>
    {credentials ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="credentials-title"><div className="my-auto max-h-[calc(100vh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl sm:p-8"><div className="mb-5 flex size-11 items-center justify-center rounded-full bg-indigo-100 text-xl text-indigo-800">✓</div><h2 id="credentials-title" className="text-2xl font-semibold tracking-tight">Сотрудник создан</h2><p className="mt-2 text-sm leading-6 text-slate-600">Сохраните данные сейчас. Временный пароль больше не будет показан.</p><div className="my-6 space-y-3 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4"><p className="text-sm font-medium text-slate-500">Логин</p><p className="break-safe font-mono text-lg font-semibold">{credentials.staff.login}</p><p className="pt-2 text-sm font-medium text-slate-500">Временный пароль</p><p className="break-safe font-mono text-lg font-semibold">{credentials.temporary_password}</p></div><div className="grid gap-2 sm:grid-cols-2"><Button type="button" onClick={() => void copyCredentials()}>Копировать данные</Button><Button type="button" variant="outline" onClick={() => setCredentials(null)}>Готово</Button></div></div></div> : null}
    <Panel title="Сотрудники">{items.map((item) => <div key={item.id} className="mb-3 rounded-xl border p-4"><div className="flex justify-between gap-3"><div><p className="font-medium">{item.display_name}</p><p className="text-sm text-slate-500">{item.login} · {item.email} · {item.role}</p>{item.role === "expert" ? <p className="text-sm">{item.public_specialist_label || "Специалист"}: {item.active_appeals}/{item.max_active_appeals}</p> : null}</div><span className="text-xs">{item.is_active ? "Активен" : "Отключён"}</span></div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => { const display_name = window.prompt("Отображаемое имя", item.display_name); const email = window.prompt("Email", item.email ?? ""); if (display_name && email) void run(() => adminApi.updateStaff(request, item.id, { display_name, email })) }}>Профиль</Button><Button size="sm" variant="outline" onClick={() => { const role = window.prompt("Роль: operator, expert или admin", item.role); if (role === "expert") { const label = window.prompt("Публичная специализация", item.public_specialist_label ?? ""); const capacity = window.prompt("Максимум активных обращений", String(item.max_active_appeals ?? 10)); if (label && capacity) void run(() => adminApi.updateStaff(request, item.id, { role, public_specialist_label: label, max_active_appeals: Number(capacity) })) } else if (role === "operator" || role === "admin") void run(() => adminApi.updateStaff(request, item.id, { role })) }}>Роль</Button><Button size="sm" variant="outline" onClick={() => { if (window.confirm("Изменить состояние сотрудника и отозвать сессии?")) void run(() => adminApi.updateStaff(request, item.id, { is_active: !item.is_active })) }}>{item.is_active ? "Деактивировать" : "Активировать"}</Button>{item.role === "expert" ? <><Button size="sm" variant="outline" onClick={() => { const value = window.prompt("Публичная специализация", item.public_specialist_label ?? ""); if (value) void run(() => adminApi.updateStaff(request, item.id, { public_specialist_label: value })) }}>Специализация</Button><Button size="sm" variant="outline" onClick={() => { const value = window.prompt("Максимум активных обращений", String(item.max_active_appeals)); if (value) void run(() => adminApi.updateStaff(request, item.id, { max_active_appeals: Number(value) })) }}>Изменить лимит</Button></> : null}</div></div>)}</Panel></>
}
