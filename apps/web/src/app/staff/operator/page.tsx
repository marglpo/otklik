...
export default function OperatorWorkspacePage() {
  ...
  return (
    <main className="staff-workspace">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 p-4 sm:p-6 lg:p-8 xl:grid-cols-[340px_1fr]">
        <aside className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {queue ? ([
              ...
              <div key={label} className={`rounded-xl border border-slate-200 p-3 shadow-sm ${style}`}><p className="text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-slate-600">{label}</p></div>
            )) : null}
          </div>
            ...
            <div className="grid gap-5 2xl:grid-cols-[1fr_320px]">
              <div className="space-y-5">
                <section className="staff-card p-5 sm:p-6">
                  <div className="flex flex-wrap items-center gap-2 text-xs"><span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 font-medium text-slate-700">{applicantTypes[detail.applicant_type] ?? detail.applicant_type}</span><StatusBadge value={detail.status} label={statuses[detail.status] ?? detail.status} /><span className="text-slate-500">Ожидает {waiting(detail.waiting_seconds)}</span>{detail.crisis_flag ? <StatusBadge value="crisis" label="Требует внимания" /> : null}</div>
              ...
              <aside className="space-y-4">
                <section className="staff-card space-y-4 border-t-4 border-t-indigo-600 p-5">
